...
public class DateTimeAPI {
	...
	private static void localDateAndTime() {
		...
		System.out.println(
				"LocalDateTime after 7 days using ChronoUnit : " + currentDateWithTime.plus(7, ChronoUnit.DAYS));
		System.out.println(
				"LocalDateTime isBefore 1 year : " + currentDateWithTime.isBefore(currentDateWithTime.plusYears(1)));
		System.out.println("LocalDateTime isAfter 7 minutes : "
				+ currentDateWithTime.isAfter(currentDateWithTime.minusMinutes(7)));
		System.out.println("LocalDateTime of 02/02/2020 : "
				+ currentDateWithTime.withDayOfMonth(2).withDayOfMonth(2).withYear(2020));

	    System.out.println("LocalDate day-month-year format parsed using DateTimeFormatter : " +
	    		DateTimeFormatter.ofLocalizedDate(FormatStyle.SHORT).format(currentDateWithTime));
	}
	...
	private static void timeAdjusters() {
		...
		System.out.printf("first day of current month : %s%n", dof);

		LocalDate lastDayOfMonth = localDate.with(TemporalAdjusters.lastDayOfMonth());
		DayOfWeek dofLm = lastDayOfMonth.getDayOfWeek();
		System.out.printf("last day of current month :  %s%n ", dofLm);

		LocalDate nextMondayDate = localDate.with(TemporalAdjusters.next(DayOfWeek.MONDAY));
		System.out.printf("next Monday : %s%n", nextMondayDate);
	}
}